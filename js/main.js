...
function openShaderDetail(shaderId) {
    ...
    // Set up navigation again to ensure header links work in detail view
    setupNavigation();
...
function setupNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            const detailContainer = document.querySelector('.shader-detail');
            if (detailContainer) {
                // We're in detail view, need to go back to main view first
                const main = document.querySelector('main');
                const previousContent = main.dataset.previousContent;
                ...
                // Clean up WebGL resources for detail view
                for (const [id, canvasInfo] of shaderCanvases.entries()) {
                    if (id.startsWith('detail-')) {
                        const shaderManager = shaderManagers.get(canvasInfo.shaderManagerKey);
                        if (shaderManager) {
                            shaderManager.deleteAllPrograms();
                            shaderManagers.delete(canvasInfo.shaderManagerKey);
                        }
                        shaderCanvases.delete(id);
                    }
                }

                // Restore previous content
                main.innerHTML = previousContent;
                ...
                // We're already in main view, just scroll to section
                const targetSection = document.getElementById(targetId);
                if (targetSection) {
                    window.scrollTo({
                        top: targetSection.offsetTop - 70, // Adjust for header height
